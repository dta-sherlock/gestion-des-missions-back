...
public class InitialiserDonneesServiceDev implements InitialiserDonneesService{
    ...
    public void Initialiser() {
         ...
         */
        if (this.natureRepository.findByNom(nature1.getName()) == null) {
            natureRepository.save(nature1);
        }

        if (this.natureRepository.findByNom(nature2.getName()) == null){
            natureRepository.save(nature2);
        }

        if (this.natureRepository.findByNom(nature3.getName()) == null){
         ...
         */
        if (this.missionRepository.findByNatureNom(mission1.getNature().getName()).isEmpty()){
            missionRepository.save(mission1);
        }

        if (this.missionRepository.findByNatureNom(mission2.getNature().getName()).isEmpty()){
            missionRepository.save(mission2);
        }

        if (this.missionRepository.findByNatureNom(mission3.getNature().getName()).isEmpty()){
